...
<a name="one">1)</a> *While reinforcement learning is exciting, it's hard to get right in the real world; i.e. outside of games and constrained environments. If you just assume that RL algorithms won't work on a given use case, you'll be right 70% of the time. There are a few reasons for that. Reinforcement learning algorithms are hard to train. They have difficulty learning from an environment how to behave better to reach their goals. That's because they depend on rewards coming from that environment, and a lot of time, those rewards are few and far between. For example, how would you train an algorithm to recognize someone you could happily marry? Or to strategize on winning a war? It won’t have a very large dataset to draw from actual experience, because you haven't married much, and even over hundreds of years we haven't fought that many wars, let alone recorded good data for them. In life, you can make a mistake and not realize it until many years later. And even then, how would you know which action out of thousands over a decade-long span led to the bad outcome? All you have are theories. One reason why reinforcement learning does so well with games is because games are easy to simulate: you can create the dataset by playing again and again. Actions in the real world aren't like that. Life is hard to replicate and the experiments that can be conducted with humans are rightfully limited. In addition, we have more choices to make in the real world, which makes for an immense garden of forking paths to cover all possibilities. With games, you might face a relatively simple choice of moving a piece here or there -- in a conversation, the possible combinations what you might say and do are nearly endless. That makes those scenarios hard to compute. So people try to narrow the range of possibilities they explore. DeepMind used Monte Carlo Tree Search with AlphaGo to narrow the beam of possibilities. PPO is another way to do that. So MCTS and PPO address part of the problem of reinforcement learning, but not the whole problem. In real life, you are still left with situations that are hard to simulate and or that grant sparse rewards. In many situations, it may be difficult to even know what the best outcome is, because you may face tradeoffs that are hard to compare: Will you be happier marrying the rich, dumb dude or the clever, poor, handsome guy? The answer is hugely important, but it is hard to know until you try, difficult to measure after the wedding, and even more difficult to predict.*
